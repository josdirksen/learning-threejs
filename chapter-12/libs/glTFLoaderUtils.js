...
THREE.GLTFLoaderUtils = Object.create(Object, {
    ...
    _loadStream: {
        value: function(path, type, delegate) {
            ...
            var isMaskedURIBlob = /data:/.test(path);
            if(isMaskedURIBlob){ // reject it; naked URIs are too dangerous //
              return;
            /*
              var reMime = /data:(.*\/.*),/;
              var mimeTYPE = path.match(reMime)[1];              
              var reFront = new RegExp("^.*,","");
              var dataStream = path.replace(reFront,"");
              var file = new Blob([dataStream],{type:mimeTYPE});
